...
function ResizerV({ onResize }) {
  ...
  useEffect(() => {
    const move = (e) => { if (dragging.current) onResize(e.clientX - startX.current); };
...
function ResizerH({ onResize }) {
  ...
  useEffect(() => {
    const move = (e) => { if (dragging.current) onResize(e.clientY - startY.current); };
    const up = () => { dragging.current = false; };
    window.addEventListener("mousemove", move);
    window.addEventListener("mouseup", up);
    return () => { window.removeEventListener("mousemove", move); window.removeEventListener("mouseup", up); };
  ...
  return (
    ...
    >
      <div className="absolute -top-1 -bottom-1 inset-x-0 group-hover:bg-blue-500/10"/>
    </div>
  );
}
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        {sidebarOpen && (
          <>
            ...
            <ResizerV onResize={(d) => setSidebarW(w => Math.max(160, Math.min(400, w + d))}/>
          </>
        )}
        ...
        <div className="flex-1 flex flex-col overflow-hidden min-w-0">
          ...
          {terminalOpen && (
            <>
              <ResizerH onResize={(d) => setTerminalH(h => Math.max(80, Math.min(500, h - d)))}/>
        ...
        {pkgPanelOpen && (
          <>
            <ResizerV onResize={(d) => setPkgPanelW(w => Math.max(200, Math.min(420, w - d)))}/>
